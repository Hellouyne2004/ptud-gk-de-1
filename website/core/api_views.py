...
from .models import BlogModel, LikeModel, FollowModel
from .serializers import LikeToggleSerializer, FollowToggleSerializer
...
class FollowToggleAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid() and request.user.is_authenticated:
            ...
            followed, created = FollowModel.objects.get_or_create(blog=blog, user=user)
            if not created:
                followed.delete()
                return Response({'message': 'Unfollowed', 'follows_count': blog.followmodel_set.count()}, status=status.HTTP_200_OK)
            else:
                return Response({'message': 'Followed', 'follows_count': blog.followmodel_set.count()}, status=status.HTTP_201_CREATED)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)