...
from core.models import BlogModel, FollowModel
...
class HomePageView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            followed_blogs = BlogModel.objects.filter(followmodel__user=self.request.user).order_by('-created_at')
...
class UserBlogListView(LoginRequiredMixin, ListView):
    model = BlogModel
    template_name = 'core/user_blogs.html'
    context_object_name = 'blogs'
    paginate_by = 10
...
class FollowedBlogsView(LoginRequiredMixin, ListView):
    model = BlogModel
    template_name = 'core/followed_blogs.html'
    context_object_name = 'blogs'
    paginate_by = 10

    def get_queryset(self):
        return BlogModel.objects.filter(followmodel__user=self.request.user).order_by('-created_at')